...
static bool FrameworkInit()
{
	...
	// Not fatal here: no UWorld/PlayerController may exist yet (still at the main menu/lobby).
	// FrameworkMainThread's loop below retries this every tick until it succeeds.
	if (!Framework::aimbotHooks.get()->Setup())
		Utils::LogDebug("AimbotHooks::Setup() failed (no active UWorld yet?), will retry.");
#endif
...
DWORD WINAPI FrameworkMainThread(LPVOID lpParam)
{
	...
	{
#if ENGINE_UNREAL
		if (!Framework::aimbotHooks.get()->IsInstalled())
			Framework::aimbotHooks.get()->Setup();
#endif
