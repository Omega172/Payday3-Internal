...
static bool GetD3D12Addresses(ID3D12Device** ppDevice, ID3D12CommandQueue** ppCommandQueue, IDXGISwapChain3** ppSwapChain, IDXGIFactory4** ppFactory)
{
	...
	// hardware adapter if WARP is unavailable) so it can't collide with the game's own
	// D3D12 device creation on the same adapter - when loaded via the Proxy DLL this runs
	// at process attach, before the engine has touched D3D12 at all, and creating/destroying
